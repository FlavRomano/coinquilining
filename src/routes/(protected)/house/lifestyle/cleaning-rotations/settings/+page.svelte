<script lang="ts">
	...
	const zones: Writable<string> = writable("");
	let checkedWeek: boolean[] = Array(7).fill(false);

	onMount(() => {
		if (cleaning_zones === null) {
			$zones = "Kitchen, Bathroom, Garden";
		} else {
			$zones = cleaning_zones.join(",");
		}

		if (cleaning_days === null) {
			checkedWeek[5] = true;
			checkedWeek[6] = true;
		} else {
			checkedWeek = [...cleaning_days];
		}
	});
</script>
...
<div class="m-5">
	<div class="text-sm breadcrumbs">
		<ul>
			...
			<li>
				<a href="/house/lifestyle/cleaning-rotations"
...
<form action="?/save" method="post">
	...
	<button
		disabled={$zones.trim() === "" ||
			$zones.split(",").filter((v) => v.trim().length !== 0).length >
				10 ||
